...
const server=http.createServer(async(req,res)=>{const origin=req.headers.origin||'';if(req.method==='OPTIONS'){res.writeHead(204,corsHeaders(origin));return res.end();}const url=new URL(req.url||'/',`http://${req.headers.host||'localhost'}`);if(url.pathname==='/health'){return json(res,200,{ok:true,service:'android-workflow-renderer',queue:queue.length,working},origin);}if(!authorizeRequest(req.headers.authorization,apiKey))return json(res,401,{error:'unauthorized'},origin);
 try{
  if(req.method==='POST'&&url.pathname==='/v1/renders'){const validation=validateRenderRequest(await readJson(req));if(!validation.ok)return json(res,400,{error:validation.error},origin);const job=store.create(validation.value);enqueue(job);return json(res,202,{job:publicJob(job)},origin);}
 ...
 }catch(error){return json(res,400,{error:error instanceof Error?error.message:'bad request'},origin);}
});
