...
const stories = [
  { name: 'Tạo tin', cover: '/facebook/user/lcd.jpg', avatar: '/facebook/user/lcd.jpg', own: true },
  { name: 'Doraemon', cover: '/facebook/story/1.jpg', avatar: '/facebook/user/doraemon.jpg' },
  { name: 'Sách Cũ Ngọc', cover: '/facebook/story/2.jpg', avatar: '/facebook/user/sachcungoc.jpg' },
  { name: 'VAFFC', cover: '/facebook/story/3.jpg', avatar: '/facebook/user/vaffc.jpg' },
];

const posts = [
  {
    user: 'Đài Phát Thanh.',
    avatar: '/facebook/user/daiphatthanh.jpg',
    time: '16 giờ',
    text: 'Rap Việt Mùa 3 (2023) đã tìm ra Top 9 bước vào Chung Kết, hứa hẹn một trận đại chiến cực căng.',
    image: '/facebook/post/1.jpg',
    reactions: '10.845',
...
const nav = [
  ...
  { id: 'dating' as const, normal: 'dating.jpg', active: 'dating-active.jpg', label: 'Hẹn hò' },
  { id: 'notifications' as const, normal: 'noti.jpg', active: 'noti-active.jpg', label: 'Thông báo' },
  ...
  return (
    <div className="bg-white text-black">
      <div className="flex items-center gap-3 px-3 py-3">
        <img src="/facebook/user/lcd.jpg" alt="Lê Công Đắt" className="h-11 w-11 rounded-full object-cover" />
    ...
    <div className="mt-2 flex gap-2 overflow-x-auto bg-white px-3 py-3 [scrollbar-width:none]">
      {stories.map((story) => (
        <button key={story.name} className="relative h-[180px] w-[104px] shrink-0 overflow-hidden rounded-xl border border-black/10 bg-white text-left shadow-sm">
          <img src={story.cover} alt="" className={`w-full object-cover ${story.own ? 'h-[125px]' : 'h-full'}`} />
          ...
          {story.own ? (
            ...
            <><img src={story.avatar} alt="" className="absolute left-2 top-2 h-10 w-10 rounded-full border-[3px] border-[#1877f2] object-cover" /><div className="absolute bottom-2 left-2 right-1 text-[12px] font-semibold leading-tight text-white">{story.name}</div></>
          )}
        </button>
      ))}
    </div>
  ...
  return (
    <article className="mt-2 bg-white pb-1 text-black">
      <div className="flex items-center gap-2.5 px-3 py-3">
        <img src={post.avatar} alt="" className="h-10 w-10 rounded-full border border-black/10 object-cover" />
      ...
      <div className="flex items-center justify-between px-3 py-2 text-[12px] text-zinc-500">
        <div className="flex items-center"><img src="/facebook/reactions/like.png" alt="" className="h-5 w-5"/><img src="/facebook/reactions/love.png" alt="" className="-ml-1 h-5 w-5"/><span className="ml-1">{post.reactions}</span></div>
