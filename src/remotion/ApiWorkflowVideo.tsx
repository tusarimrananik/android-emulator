...
export type ApiWorkflowVideoProps = {actions: WorkflowAction[]; fps?: 15|30|60};
...
export const ApiWorkflowVideo: React.FC<ApiWorkflowVideoProps> = ({actions}) => {
  ...
  let currentApp:string|undefined; let tapValue:string|undefined; let drawer=false; let scroll=0; let cursor=0;
  for(const action of actions){if(cursor>=active.start) break; if(action.type==='openDrawer'||action.type==='scrollDrawer') drawer=true; if(action.type==='scrollDrawer') scroll=-560; if(action.type==='openApp'){currentApp=action.app;drawer=false;} if(action.type==='tap') tapValue=action.value; if(action.type==='goHome'||action.type==='home'){currentApp=undefined;drawer=false;} cursor+=Math.max(1,Math.round(action.duration*fps));}
